...
class XmlDecoder extends DefaultHandler {
	...
	public void endElement(String uri, String localName, String qName) {
		...
		if (qName == "next_sim") {
			...
			pcmSwitch = str.equals("true");
		} else if (qName == "melting_point") {
			meltingPoint = Float.parseFloat(str);
		} else if (qName == "latent_heat") {
			latentHeatFusion = Float.parseFloat(str);
		} else if (qName == "specific_heat_solid") {
			specificHeatSolid = Float.parseFloat(str);
		} else if (qName == "specific_heat_liquid") {
			specificHeatLiquid = Float.parseFloat(str);
		} else if (qName == "part") {
			if (part != null) {
				...
				part.setUid(uid);
				part.setLabel(label);
				part.setPcm(pcmSwitch);
				part.setMeltingPoint(meltingPoint);
				part.setLatentHeatFusion(latentHeatFusion);
	...
	private void resetPartVariables() {
		...
		pcmSwitch = false;
		meltingPoint = 0f;
		latentHeatFusion = 336000f;
		specificHeatLiquid = 4154f;
		specificHeatSolid = 4154f;
	}
