...
class PartModelDialog extends JDialog {
	...
	//pcm bits
	private JRadioButton pcmButton, notPcmButton;
	private JTextField specificHeatSolidField, specificHeatLiquidField, latentHeatField, meltingPointField;
	...
	PartModelDialog(final View2D view, final Part part, boolean modal) {
		...
		okListener = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				float specificHeatSolid = parse(specificHeatSolidField.getText());
				if (Float.isNaN(specificHeatSolid))
					return;
				if (specificHeatSolid <= 0) {
					JOptionPane.showMessageDialog(owner, "Specific heat must be greater than zero.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				float specificHeatLiquid = parse(specificHeatLiquidField.getText());
				if (Float.isNaN(specificHeatLiquid))
					return;
				if (specificHeatLiquid <= 0) {
					JOptionPane.showMessageDialog(owner, "Specific heat must be greater than zero.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				float latentHeat = parse(latentHeatField.getText());
				if (Float.isNaN(latentHeat))
					return;
				if (latentHeat <= 0) {
					JOptionPane.showMessageDialog(owner, "The latent heat of fusion must be greater than zero.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				float meltingPoint = parse(meltingPointField.getText());
				if (Float.isNaN(meltingPoint))
					return;
				...
				part.setUid(uid);
				part.setSpecificHeatSolid(specificHeatSolid);
				part.setSpecificHeatLiquid(specificHeatLiquid);
				part.setLatentHeatFusion(latentHeat);
				part.setMeltingPoint(meltingPoint);
				part.setPcm(pcmButton.isSelected() && !notPcmButton.isSelected());
		...
		bg = new ButtonGroup();
		notPcmButton = new JRadioButton("Not a phase change material");
		notPcmButton.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					specificHeatField.setEnabled(true);
					specificHeatSolidField.setEnabled(false);
					specificHeatLiquidField.setEnabled(false);
					latentHeatField.setEnabled(false);
					meltingPointField.setEnabled(false);
				}
			}
		});
		p.add(notPcmButton);
		...
		pcmButton = new JRadioButton("Phase change material (experimental)");
		pcmButton.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					specificHeatField.setEnabled(false);
					specificHeatSolidField.setEnabled(true);
					specificHeatLiquidField.setEnabled(true);
					latentHeatField.setEnabled(true);
					meltingPointField.setEnabled(true);
				}
			}
		});
		p.add(pcmButton);
		...
		p.add(new JLabel("Specific heat (not pcm)"));
		...
		p.add(new JLabel("Specific heat (solid)"));
		specificHeatSolidField = new JTextField(FORMAT.format(part.getSpecificHeatSolid()), 8);
		specificHeatSolidField.addActionListener(okListener);
		p.add(specificHeatSolidField);
		p.add(new JLabel("<html><i>J/(kg\u00b7\u2103)</i></html>"));
		count++;

		p.add(new JLabel("Specific heat (liquid)"));
		specificHeatLiquidField = new JTextField(FORMAT.format(part.getSpecificHeatLiquid()), 8);
		specificHeatLiquidField.addActionListener(okListener);
		p.add(specificHeatLiquidField);
		p.add(new JLabel("<html><i>J/(kg\u00b7\u2103)</i></html>"));
		count++;

		p.add(new JLabel("Latent heat of fusion"));
		latentHeatField = new JTextField(FORMAT.format(part.getLatentHeatFusion()), 8);
		latentHeatField.addActionListener(okListener);
		p.add(latentHeatField);
		p.add(new JLabel("<html><i>J/kg</i></html>"));
		count++;

		p.add(new JLabel("Melting point"));
		meltingPointField = new JTextField(FORMAT.format(part.getMeltingPoint()), 8);
		meltingPointField.addActionListener(okListener);
		p.add(meltingPointField);
		p.add(new JLabel("<html><i>\u2103</i></html>"));
		count++;
