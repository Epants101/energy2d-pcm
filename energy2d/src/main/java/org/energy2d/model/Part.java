...
public class Part extends Manipulable {
	...
	//pcm properties
	private boolean pcmSwitch = false;
	private float meltingPoint = 0f;
	private float specificHeatSolid = 4154f;
	private float specificHeatLiquid = 4154f;
	//Unit: J/kg
	private float latentHeatFusion = 336000f;
	//pcm calculation properties
	private boolean melted = false;
	private float usedEnergy = 0;
	private float storedEnergyPerKg;
	...
	public float getStoredEnergy() {
		return storedEnergyPerKg*getMass();
	}

	//This function should only be used for pcms. this assumes that the power will only be applied for one step
	public float powerForTemp(float Temperature) {
		float powerT;
		//(m * c * DeltaT / time)
		powerT = ((density * specificHeat * (Temperature - averageTemp())) / model.heatSolver.timeStep);
		return powerT;
	}

	public float averageTemp() {
		//0.01 is a magic number
		float temp = (model.getThermalEnergy(this)) / (density * specificHeat * getArea());
		return temp;
	}
	...
	public void setUsedEnergy(float previousEnergy) {
		this.usedEnergy = previousEnergy;
	}

	public float getStoredEnergyPerKg() {
		return storedEnergyPerKg;
	}

	public void setStoredEnergyPerKg(float storedEnergy) {
		this.storedEnergyPerKg = storedEnergy;
	}
	...
	public void setMelted(boolean melted) {
		...
		if (pcmSwitch) {
			if (melted) {
				this.usedEnergy = 0;
				specificHeat = specificHeatLiquid;
			} else {
				this.usedEnergy = latentHeatFusion * getMass();
				specificHeat = specificHeatSolid;
		}}
	...
	public void setPcm(boolean pcmSwitch) {
		this.pcmSwitch = pcmSwitch;
		if (pcmSwitch)
			temperature = meltingPoint;
	}

	public float getLatentHeatFusion() {
		return latentHeatFusion;
	}

	public void setLatentHeatFusion(float latentHeatFusion) {
		this.latentHeatFusion = latentHeatFusion;
		if (!melted) 
			this.usedEnergy = latentHeatFusion * getMass();
	}

	public float getSpecificHeatLiquid() {
		return specificHeatLiquid;
	}

	public void setSpecificHeatLiquid(float specificHeatLiquid) {
		this.specificHeatLiquid = specificHeatLiquid;
		if (melted && pcmSwitch) 
			specificHeat = specificHeatLiquid;
	}

	public float getSpecificHeatSolid() {
		return specificHeatSolid;
	}

	public void setSpecificHeatSolid(float specificHeatSolid) {
	...
	public void copyPropertiesTo(Part p) {
		...
		p.meltingPoint = meltingPoint;
		p.specificHeatLiquid = specificHeatLiquid;
		p.specificHeatSolid = specificHeatSolid;
		p.latentHeatFusion = latentHeatFusion;
		p.pcmSwitch = pcmSwitch;
		p.setLabel(getLabel());
	}
	...
	public String toXml() {
		...
		if (isPcm())
			xml += "<pcm>true</pcm>\n";
			xml += "<latent_heat>" + latentHeatFusion + "</latent_heat>\n";
			xml += "<specific_heat_solid>" + specificHeatSolid + "</specific_heat_solid>\n";
			xml += "<specific_heat_liquid>" + specificHeatLiquid + "</specific_heat_liquid>\n";
			xml += "<melting_point>" + meltingPoint + "</melting_point>\n";
		xml += "</part>\n";
		return xml;
	}
