...
public class Model2D {
    ...
    public HeatSolver2D heatSolver;
    ...
    private void managePcms() {
        //required local variables
        float powerForMelting;
        for (Part part : parts) {
            //For each pcm
            if (part.isPcm()) {
                //System.out.println(part.averageTemp());
                if (!part.getPowerSwitch())
                    part.setPowerSwitch(true);
                //System.out.println(part.powerForTemp(part.getMeltingPoint()));
                powerForMelting = part.powerForTemp(part.getMeltingPoint());
                //If it is above its melting point and it is not melted
                if ((part.averageTemp() > part.getMeltingPoint()) && (part.getUsedEnergy() > 0)) {
                    if (part.getUsedEnergy() > (-1 * powerForMelting * heatSolver.timeStep * part.getArea())) {
                        part.setPower(powerForMelting);
                        part.setUsedEnergy(part.getUsedEnergy() + (powerForMelting * heatSolver.timeStep * part.getArea()));
                    } else {
                        part.setPower(-1 * part.getUsedEnergy() / (part.getArea() * heatSolver.timeStep));
                        part.setUsedEnergy(0);
                        part.setMelted(true);
                    }
                } else if ((part.averageTemp() < part.getMeltingPoint()) && (part.getUsedEnergy() < (part.getLatentHeatFusion() * part.getMass()))) {
                    if (((part.getLatentHeatFusion() * part.getMass()) - part.getUsedEnergy()) > (powerForMelting * heatSolver.timeStep * part.getArea())) {
                        part.setPower(powerForMelting);
                        part.setUsedEnergy(part.getUsedEnergy() + (powerForMelting * heatSolver.timeStep * part.getArea()));
                    } else {
                        part.setPower(((part.getLatentHeatFusion() * part.getMass()) - part.getUsedEnergy()) / (part.getArea() * heatSolver.timeStep));
                        part.setUsedEnergy(part.getLatentHeatFusion() * part.getMass());
                        part.setMelted(false);
                    }
                } else {
                    part.setPower(0);
                }
